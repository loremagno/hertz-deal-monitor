...
Working end to end, run locally by hand. Not yet running on a schedule.
...
- `.github/workflows/monitor.yml` — **gitignored**, cannot be pushed (scope)
...
### Git Status
- Branch `main` @ `38cffb1`, pushed, **clean working tree**.
- Remote `github.com/LorenzoMagnolfi/hertz-deal-monitor`, **public** since
  2026-09-09 (made public at Lorenzo's request after a full history scan for
  secrets came back clean).
...
## Pending / Next Steps
- [ ] **Install the Actions workflow.** `deploy/monitor-workflow.yml` →
      `.github/workflows/monitor.yml`, via the GitHub web UI or after widening
      the credential to `workflow` scope. Nothing runs on a schedule until this
      is done. See `deploy/README.md`.
- [ ] **Enable GitHub Pages** (Settings → Pages → `main` / `/out`) for a
      self-updating dashboard URL. My API attempt was blocked by a local
      permission classifier, not by GitHub.
- [ ] **Watch the first Actions run.** Unverified whether Akamai blocks a
      datacenter IP. Fallback if so: a small VPS, or run locally on a schedule.
...
- **Repo is public and contains his home zip (43220) and car preferences.**
  Flagged to him; he asked for public anyway (free Pages).
